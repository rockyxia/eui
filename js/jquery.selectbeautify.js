	...
	cssSelect = function (config) {
		...
		return $(this).each(function(){
			...
			var	$thisEl = $(this),
				...
				/* Ui */
				tpl = {
					wrapper: '<div class="eui-select-box"></div>',
					select: '<div class="selectBox sNormal" style="width:'+($thisEl.width()+36)+'px"></div>',
					selectLt: '<div class="selectLt"></div>',
					selectRt: '<div class="selectRt"></div>',
					options: '<div class="optionsBox" style="width:'+($thisEl.width()+36)+'px"></div>',
					optionsInner: '<div class="optionsI-nnerBox"></div>',
				...
				init = function() {
					...
					)
					{
						...
						/* Render select*/
						$selectEl.append($sRtEl).append($sLtEl);
						...
						$thisEl.setOptions = function(){
							...
							$optionsEl.css({ 'top' : top-2 , 'left' : sltPosition.left, 'overflow-y': 'auto', 'overflow-x': 'hidden'});
